...
def reverse_weight(weight):

    # 将tensor转换为NumPy数组
    original_array = weight.numpy()

    # 取倒数
    inverse_array = 1 / original_array

    # 归一化，使得所有元素的和为1
    normalized_array = inverse_array / np.sum(inverse_array)
...
class GaussianDiffusionTrainer(nn.Module):
    ...
    def forward(self, x_0, y_0, augm=None):
        ...
        loss_reg = loss_com = torch.tensor(0).to(x_t.device)  # L_r  L_rc
        weight_p = reverse_weight(self.weight)
        if self.cb and y_0 is not None:
            y_bal = torch.Tensor(np.random.choice(  # y_train ?
                                 self.num_class, size=len(x_0),
                                 p=weight_p if not self.finetune else None,
        ...
        return loss, loss_reg + 1/4 * loss_com  # 1/4 -> γ
